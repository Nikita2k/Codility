...
NSMutableArray * solution(NSMutableArray *A, NSMutableArray *B) {
    ...
    for (int i = 2; i <= [A count]; i++) {
        NSInteger currentVal = [ladder[i-2] integerValue] + [ladder[i-1] integerValue];
        [ladder addObject:[NSNumber numberWithInteger:currentVal]];
    }
    ...
    for (int i = 0; i < [A count]; i++) {
        ...
        //        double val = [ladder[[A[i] integerValue]] integerValue] % (NSInteger)(pow(2, [B[i] integerValue]));
        double val1 = [ladder[[A[i] integerValue]] integerValue] & ((1 << [B[i] integerValue]) - 1);
        [res addObject:[NSNumber numberWithDouble:val1]];
    }
